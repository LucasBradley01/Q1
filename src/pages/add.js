...
var worker = require("../workers/add");
...
module.exports = {
    oninit: () => {
        utl.init();
        worker.loadLayer(m.route.param("index"));
    },
    view: () => {
        return m("div[class=root]", [
            ...
            m("div[class=header]", [
                m("div", "Add Data to " + (JSON.parse(localStorage.getItem("state"))).content[m.route.param("index")].name),
            ]),
            ...
            m("div[class=three-section]", [
                m("button", {
                    onclick: () => {
                        worker.add(m.route.param("index"), formInput);
                    }}, "Update"),
                m("button", {
                    onclick: () => {
                        formInput.longitude = undefined;
                        formInput.latitude = undefined;
                        worker.fields.map((item) => {
                            if (item.name != "OBJECTID") {
